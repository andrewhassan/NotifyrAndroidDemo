...
public class NLService extends NotificationListenerService {
    ...
    public void onNotificationPosted(StatusBarNotification sbn) {
        ...
        if(sbn.isClearable()){
            ...
            BLEConnectionHandler.getInstance().writeMessage(outputValue,sbn.getNotification().tickerText.length()+1);

            BluetoothDevice device = m_bt_adapter.getRemoteDevice(address);
            if (device != null && !BLEConnectionHandler.getInstance().isWriting()) {
